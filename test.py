...
if uploaded_image1 is not None:
    ...
    with col1:
        resized_img = image1.resize((150, 150))
        st.image(resized_img)

    # with col2:
    #     if st.button('Classify1'):
    #         # Apply the transformation to the image
    #         processed_image = transformation(image1).unsqueeze(0)  # Add a batch dimension
    #         output1, output2 = net(processed_image, processed_image)
    #         euclidean_distance = F.pairwise_distance(output1, output2)
    #         # Preprocess the uploaded image and predict the class
    #         st.success(f'Prediction: {euclidean_distance.item():.2f}')
...
if uploaded_image2 is not None:
    ...
    with col1:
        resized_img = image2.resize((150, 150))
        st.image(resized_img)

    with col2:
        if st.button('Go..'):
             # Apply the transformation to the image
            processed_image1 = transformation(image1).unsqueeze(0)  # Add a batch dimension
            processed_image2 = transformation(image2).unsqueeze(0)  # Add a batch dimension
            output1, output2 = net(processed_image1, processed_image2)
            euclidean_distance = F.pairwise_distance(output1, output2)
            # Preprocess the uploaded image and predict the class
            st.success(f'درجة الاختلاف: {euclidean_distance.item():.2f}')
